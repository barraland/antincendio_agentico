...
# ============================================================
# DEPLOY "antincendio-agentico" su Azure Container Apps
# - HTTP ingress su porta 8000 (external)
...
APP_NAME="antincendio-agentico-container"
APP_NAME_FALLBACK="antincendio-agentico-container"
...
LLAMA_CLOUD_API_KEY="$(get_env LLAMA_CLOUD_API_KEY || true)"

# Required
require_var "OPENAI_API_KEY"

# Optional + defaults
RAG_MODEL="$(get_env_default RAG_MODEL gpt-5-mini)"
RAG_ROUTER_MODEL="$(get_env_default RAG_ROUTER_MODEL gpt-5-mini)"
RAG_SELECTOR_MODEL="$(get_env_default RAG_SELECTOR_MODEL gpt-5-mini)"
...
echo "🔎 Check rete ACR..."
az acr show -n "$ACR_NAME" --query "{publicNetworkAccess:publicNetworkAccess, defaultAction:networkRuleSet.defaultAction}" -o json || true

# ----------------------------
# Choose app (fallback if Failed)
# ----------------------------
CHOSEN_APP="$APP_NAME"
if az containerapp show -g "$RG" -n "$APP_NAME" >/dev/null 2>&1; then
  STATE="$(az containerapp show -g "$RG" -n "$APP_NAME" --query properties.provisioningState -o tsv || true)"
  if [[ "$STATE" == "Failed" ]]; then
    CHOSEN_APP="$APP_NAME_FALLBACK"
  fi
fi
echo "➡️ App target: $CHOSEN_APP"

# ----------------------------
# Create app if missing (with HTTP ingress on port 8000)
# ----------------------------
if ! az containerapp show -g "$RG" -n "$CHOSEN_APP" >/dev/null 2>&1; then
  echo "🆕 Creo app con dummy image (HTTP ingress, porta $TARGET_PORT)..."
  ...
  if ! wait_provisioning "$CHOSEN_APP" 900 5; then
    dump_diagnostics "$CHOSEN_APP"
...
az containerapp identity assign -g "$RG" -n "$CHOSEN_APP" --system-assigned >/dev/null

PRINCIPAL_ID="$(az containerapp show -g "$RG" -n "$CHOSEN_APP" --query identity.principalId -o tsv)"
...
az containerapp registry set -g "$RG" -n "$CHOSEN_APP" --server "$ACR_SERVER" --identity system >/dev/null

# ----------------------------
# Update image (forza nuova revisione)
# ----------------------------
echo "♻️ Aggiorno immagine..."
az containerapp update -g "$RG" -n "$CHOSEN_APP" --image "$IMAGE" --no-wait >/dev/null
...
az containerapp secret set -g "$RG" -n "$CHOSEN_APP" --secrets "openai-api-key=$OPENAI_API_KEY" >/dev/null
if [[ -n "${LLAMA_CLOUD_API_KEY:-}" ]]; then
  az containerapp secret set -g "$RG" -n "$CHOSEN_APP" --secrets "llama-cloud-api-key=$LLAMA_CLOUD_API_KEY" >/dev/null
fi
echo "✅ Secrets impostati"

# ----------------------------
# Env vars + force new revision
# ----------------------------
...
ENV_VARS=(
  ...
  RAG_MODEL="$RAG_MODEL"
  RAG_ROUTER_MODEL="$RAG_ROUTER_MODEL"
  RAG_SELECTOR_MODEL="$RAG_SELECTOR_MODEL"
  DEPLOY_TS="$DEPLOY_TS"
)

if [[ -n "${LLAMA_CLOUD_API_KEY:-}" ]]; then
  ENV_VARS+=(LLAMA_CLOUD_API_KEY="secretref:llama-cloud-api-key")
fi

az containerapp update -g "$RG" -n "$CHOSEN_APP" --set-env-vars "${ENV_VARS[@]}" --no-wait >/dev/null
...
CUR_IMAGE="$(az containerapp show -g "$RG" -n "$CHOSEN_APP" --query properties.template.containers[0].image -o tsv)"
CUR_REV="$(az containerapp show -g "$RG" -n "$CHOSEN_APP" --query properties.latestRevisionName -o tsv)"
FQDN="$(az containerapp show -g "$RG" -n "$CHOSEN_APP" --query properties.configuration.ingress.fqdn -o tsv)"
...
echo
echo "Logs:"
echo "  az containerapp logs show -g $RG -n $CHOSEN_APP --follow"
